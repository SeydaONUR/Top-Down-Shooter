...
public class PlayerController : MonoBehaviour
{
    ...
    public GameObject[] bulletBack;//Number of bullet
    ...
    void Update()
    {
        ...
        if (!canShot)
        {
            ...
            if (currentTime >=weapons[currentWeapon].waitToShot ) // Silahlarýn ates etme suresýný bekle
            {
                currentTime = 0;
                canShot = true;
            }
    ...
    public void ChangeWeapon()
    {
        ...
        if (Input.GetKeyDown(KeyCode.LeftShift))//Silah deðiþtiriyorum
        {
            ...
            {
                currentWeapon = 0;  // ilk silaha geri dondu
            }
    ...
    public void Shoot()
    {
        if ((Input.GetButtonDown("Fire1") || Input.GetButton("Fire1")) && canShot == true && bulletNumber[currentWeapon] > 0)
        {
            ...
            bulletNumber[currentWeapon] -= 1;// ates ettikce mermi azaltiyorum
            canShot = false;
        }
    }
    public void Molotov()
    {
        if (Input.GetKeyDown(KeyCode.Mouse1) && canBomb == true) // Just one 
        {
